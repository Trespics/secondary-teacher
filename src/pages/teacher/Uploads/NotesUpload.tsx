...
import { FileText, BookMarked, UploadCloud, CheckCircle2, X, Loader2, AlertCircle, File } from "lucide-react";
...
export const NotesUpload: React.FC<NotesUploadProps> = ({ classes, onSuccess, editingMaterial, onCancel }) => {
  const {
    form,
    setForm,
    uploadedFiles,
    ...
    handleFileUpload,
    handleSubmit,
  } = useUploadLogic("Notes", editingMaterial, onSuccess, onCancel);
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card className="border-slate-100 shadow-sm">
              <CardHeader className="bg-slate-50/50 border-b border-slate-100">
                ...
                <CardDescription>Enter the core details of your lesson notes</CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      placeholder={uploadedFiles.length > 1 ? "Will use filenames as titles" : "e.g., Introduction to Calculus"}
                      value={form.title}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    placeholder="calculus, math, form4"
                    value={form.tags}
          ...
          <div className="space-y-6">
            <Card className="border-slate-100 shadow-sm overflow-hidden">
              ...
              <CardContent className="p-6 space-y-6">
                 <div className="space-y-3">
                  <div 
                    ...
                    className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all hover:bg-slate-50 ${uploading ? 'bg-blue-50 border-blue-200' : 'border-slate-200'}`}
                  >
                    <input type="file" id="notes-file" className="hidden" accept=".pdf" multiple onChange={handleFileUpload} />
                    {uploading ? (
                      <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs font-medium">Click to upload PDF(s)</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {uploadedFiles.length > 0 && (
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold uppercase text-slate-500">Uploaded Files ({uploadedFiles.length})</Label>
              ...
              <CardFooter className="bg-slate-50/50 p-6">
                <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 font-bold" disabled={isSubmitting || uploading || (uploadedFiles.length === 0 && !form.content_link)}>
                  {isSubmitting ? (
                    <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Publishing...</>
                  ) : (
                    <><UploadCloud className="w-4 h-4 mr-2" /> {uploadedFiles.length > 1 ? `Publish ${uploadedFiles.length} Notes` : "Publish Notes"}</>
                  )}
                </Button>
              </CardFooter>
            </Card>
