...
import { Headphones, BookMarked, UploadCloud, CheckCircle2, Loader2, X, File } from "lucide-react";
...
export const AudioUpload: React.FC<AudioUploadProps> = ({ classes, onSuccess, editingMaterial, onCancel }) => {
  const {
    form,
    setForm,
    uploadedFiles,
    ...
    handleFileUpload,
    handleSubmit,
  } = useUploadLogic("Audio", editingMaterial, onSuccess, onCancel);
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card className="border-slate-100 shadow-sm">
              <CardHeader className="bg-slate-50/50 border-b border-slate-100">
                ...
                <CardDescription>Enter the core details of your audio recording or podcast</CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      placeholder={uploadedFiles.length > 1 ? "Will use filenames as titles" : "e.g., Oral Literature Analysis"}
                      value={form.title}
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    placeholder="Describe what this audio covers..."
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    placeholder="literature, english, audio"
                    value={form.tags}
                ...
                {!form.class_id ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2 md:col-span-2">
                      ...
                      <Select value={form.learning_outcome_id} onValueChange={(val) => setForm({ ...form, learning_outcome_id: val })} disabled={!form.sub_strand_id}>
                        <SelectTrigger><SelectValue placeholder="Select outcome" /></SelectTrigger>
                        <SelectContent>
                          {learningOutcomes.map(lo => <SelectItem key={lo.id} value={lo.id}>{lo.description}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            <Card className="border-slate-100 shadow-sm overflow-hidden">
              ...
              <CardContent className="p-6 space-y-6">
                 <div className="space-y-3">
                  <div 
                    ...
                    className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all hover:bg-slate-50 ${uploading ? 'bg-pink-50 border-pink-200' : 'border-slate-200'}`}
                  >
                    <input type="file" id="audio-file" className="hidden" accept="audio/*" multiple onChange={handleFileUpload} />
                    {uploading ? (
                      <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs font-medium">Click to upload Audio(s)</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {uploadedFiles.length > 0 && (
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold uppercase text-slate-500">Uploaded Audios ({uploadedFiles.length})</Label>
                      ...
                      {uploadedFiles.map((file, idx) => (
                        <div key={idx} className="flex items-center justify-between p-2 rounded-lg border border-slate-100 bg-slate-50/50 group">
                          <div className="flex items-center gap-2 overflow-hidden">
                            <File className="w-3 h-3 text-pink-500 flex-shrink-0" />
              ...
              <CardFooter className="bg-slate-50/50 p-6">
                <Button type="submit" className="w-full bg-pink-600 hover:bg-pink-700 font-bold" disabled={isSubmitting || uploading || (uploadedFiles.length === 0 && !form.content_link)}>
                  {isSubmitting ? (
                    <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Publishing...</>
                  ) : (
                    <><UploadCloud className="w-4 h-4 mr-2" /> {uploadedFiles.length > 1 ? `Publish ${uploadedFiles.length} Audios` : "Publish Audio"}</>
                  )}
                </Button>
              </CardFooter>
            </Card>
