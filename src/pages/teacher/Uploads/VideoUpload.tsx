...
import { Video, BookMarked, UploadCloud, CheckCircle2, Loader2, X, File } from "lucide-react";
...
export const VideoUpload: React.FC<VideoUploadProps> = ({ classes, onSuccess, editingMaterial, onCancel }) => {
  const {
    form,
    setForm,
    uploadedFiles,
    ...
    handleFileUpload,
    handleSubmit,
  } = useUploadLogic("Video", editingMaterial, onSuccess, onCancel);
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card className="border-slate-100 shadow-sm">
              <CardHeader className="bg-slate-50/50 border-b border-slate-100">
                ...
                <CardDescription>Enter the core details of your video lecture or tutorial</CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      placeholder={uploadedFiles.length > 1 ? "Will use filenames as titles" : "e.g., Understanding Photosynthesis"}
                      value={form.title}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    placeholder="biology, science, tutorial"
                    value={form.tags}
          ...
          <div className="space-y-6">
            <Card className="border-slate-100 shadow-sm overflow-hidden">
              ...
              <CardContent className="p-6 space-y-6">
                 <div className="space-y-3">
                  <div 
                    ...
                    className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all hover:bg-slate-50 ${uploading ? 'bg-purple-50 border-purple-200' : 'border-slate-200'}`}
                  >
                    <input type="file" id="video-file" className="hidden" accept="video/*" multiple onChange={handleFileUpload} />
                    {uploading ? (
                      <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs font-medium">Click to upload Video(s)</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {uploadedFiles.length > 0 && (
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold uppercase text-slate-500">Uploaded Videos ({uploadedFiles.length})</Label>
                      ...
                      {uploadedFiles.map((file, idx) => (
                        <div key={idx} className="flex items-center justify-between p-2 rounded-lg border border-slate-100 bg-slate-50/50 group">
                          <div className="flex items-center gap-2 overflow-hidden">
                            <File className="w-3 h-3 text-purple-500 flex-shrink-0" />
              ...
              <CardFooter className="bg-slate-50/50 p-6">
                <Button type="submit" className="w-full bg-purple-600 hover:bg-purple-700 font-bold" disabled={isSubmitting || uploading || (uploadedFiles.length === 0 && !form.content_link)}>
                  {isSubmitting ? (
                    <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Publishing...</>
                  ) : (
                    <><UploadCloud className="w-4 h-4 mr-2" /> {uploadedFiles.length > 1 ? `Publish ${uploadedFiles.length} Videos` : "Publish Video"}</>
                  )}
                </Button>
              </CardFooter>
            </Card>
